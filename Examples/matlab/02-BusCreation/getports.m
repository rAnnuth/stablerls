...
function getports(model)
    ...
    for i = 1 : length(top_ports)
        ...
        % TODO check with is equal if signal exists
        ...
        for i4 = 1 : length(top_ports)
            if isequal(get_param(top_ports{i4}, 'OutDataTypeStr'), source)
                ...
                if isequal(get_param(top_ports{i},'BlockDescription'), ...
                    ...
                    warning(['I had to rename the Bus! Ignore the name above ' ...
                        'for the following port! Unable to set DataType for "%s" due to Matlab ' ...
                        'limitations. Please set manually to "%s" or use normal ' ...
                        'inports at the top level\n Use hilite_system("%s") ' ...
                        'to find the block'], ...
                        top_ports{i}, ['Bus: bus' char(string(num + increment))], top_ports{i})
