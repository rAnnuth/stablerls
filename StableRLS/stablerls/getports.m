...
function getports(model)
    ...
    for i = 1 : length(top_ports)
        ...
        % now we set the datatype of the ports
        % check if we found a input port or bus element
    
    %     % we iterate through all ports to find other ports with the same number
    %     for i2 = 1 : length(top_ports)
    %     portnumber2 = str2double(get_param(top_ports{i2},'Port'));
    %     % only continue if port matches
    %     if ~isequal(portnumber, portnumber2)
    %         continue
    %     end
    %     name = get_param(top_ports{i},'Element');
    % 
    %     % check if we found an inport or an bus element without name
    %     if isempty(name)
    %         % use description to identify bus elements
    %         if isequal(get_param(top_ports{i},'BlockDescription'), ...
    %                 ['Select elements of a bus or the entire bus, signal, ' ...
    %                 'or message from the input port.'])      
    %         else
    %         
    %         end
    %     
    %     else
    %         for i3 = 1 : length(bus{end})
    %             if isequal(name, bus{end}{i3}{1})
    %             end
    %         end
    %         
    % 
    ...
    for i3 = 1 : length(top_ports)
        ...
        if isequal(dst_type,'SubSystem')
            ...
            if is_bdroot
                % if this is true we have a empty bus element or inport 
                if isempty(get_param(top_ports{i3},'Element'))
                    % set corret bus type
                    set_param(top_ports{i3},'OutDataTypeStr',['Bus: bus' char(string(num))]);
                % if this is true we have a bus element 
                else
                    % set corret bus type                
