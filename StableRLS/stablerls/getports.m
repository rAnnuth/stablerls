...
saveChanges(DataDict);
    ...
    for i3 = 1 : length(top_ports)
        ...
        %  PS Converters behave similar to subsystems in newer matlab releases
        dst_name = get_param(handle_dest_block,'Name');
        if contains(dst_name,"Simulink-PS")
            continue
        end
        ...
        if ~is_bdroot
            [~, bus_port]= get_name(top_ports{i3});
